...
def parse_args():
    ...
    parser.add_argument("config", type=str, help="root path of benchmarked configs to be gathered")
    parser.add_argument("out_dir", type=str, help="output path of gathered models to be stored")
    parser.add_argument("out_file", type=str, help="the output json file name")
    parser.add_argument("--filter", type=str, nargs="+", default=[], help="config filter")
...
def main():
    ...
    for model in model_infos:
        model_publish_dir = osp.join(models_out, model["raw_config"].rstrip(".py"))
        model_name = osp.split(model["config"])[-1].split(".")[0]

        publish_model_path = osp.join(model_publish_dir, model_name + "_" + model["model_time"])
        trained_model_path = osp.join(
            models_root, model["config"], "iter_{}.pth".format(model["iters"])
        )
