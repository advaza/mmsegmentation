...
def get_final_results(log_json_path, iter_num):
    ...
    with open(log_json_path, "r") as f:
        for line in f.readlines():
            ...
            if log_line["mode"] == "train" and log_line["iter"] == iter_num - 50:
                result_dict["memory"] = log_line["memory"]

            if log_line["iter"] == iter_num:
                result_dict.update(
                    {key: log_line[key] * 100 for key in RESULTS_LUT if key in log_line}
                )
                return result_dict
...
def main():
    ...
    with open(osp.join(models_out, "models_table.csv"), "w", newline="") as csvfile:
